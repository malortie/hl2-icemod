...
{
	// Cheat to create a lynx in front of the player
	Vector vecForward;
	AngleVectors( pPlayer->EyeAngles(), &vecForward );
	CBaseEntity *pLynx = (CBaseEntity *)CreateEntityByName( "prop_vehicle_lynx" );
	if ( pLynx )
	{
		Vector vecOrigin = pPlayer->GetAbsOrigin() + vecForward * 256 + Vector(0,0,64);
		QAngle vecAngles( 0, pPlayer->GetAbsAngles().y - 90, 0 );
		pLynx->SetAbsOrigin( vecOrigin );
		pLynx->SetAbsAngles( vecAngles );
		pLynx->KeyValue( "model", "models/vehicles/snowmobile.mdl" );
		pLynx->KeyValue( "solid", "6" );
		pLynx->KeyValue( "targetname", "jeep" );
		pLynx->KeyValue( "vehiclescript", "scripts/vehicles/snowmobile.txt" );
		DispatchSpawn( pLynx );
		pLynx->Activate();
		pLynx->Teleport( &vecOrigin, &vecAngles, NULL );
	}
}
...
static ConCommand ch_createlynx("ch_createlynx", CC_CH_CreateLynx, "Spawn a snowmobile in front of the player.", FCVAR_CHEAT);
